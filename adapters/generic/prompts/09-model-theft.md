...
This is a standalone threat analysis prompt for the **Model Theft** threat category.

**Input**: Provide the architecture components assigned to this agent by the orchestrator (from Step 9 of `00-orchestrator.md`).
**Output**: A list of findings in the structured format specified below.

**Conditional activation**: This agent only runs when architecture components match LLM/AI-related trigger keywords (see Detection Scope below). If the orchestrator determined no components match, skip this prompt entirely.

Paste the relevant architecture components below, then submit this entire prompt to your LLM.
...
- **OWASP LLM06:2026 - Unbounded Consumption** (the standalone Model Theft entry was folded into this category at the 2025 edition): https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/
