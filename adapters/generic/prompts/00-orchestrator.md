...
This is a **sequential workflow guide** for running a complete STRIDE + AI threat model using any LLM. There is no automated agent dispatch -- you execute each analysis step manually by copy-pasting prompts in order.

**Workflow**:

1. Copy-paste this orchestrator prompt into your LLM conversation first.
2. Provide your architecture diagram or description where indicated by `{{ARCHITECTURE_INPUT}}` below.
3. The orchestrator will complete Phase 1 (Scope) and Phase 2 (dispatch planning).
4. Then, run each threat agent prompt sequentially as numbered steps (see "Sequential Workflow Steps" section).
5. After all agents complete, return to this orchestrator context for Phase 3 (Countermeasures), Phase 4 (Assessment), and Phase 5 (Report).

Each threat agent prompt is a standalone file. Run them in order, passing the architecture context and target components from Phase 2 into each prompt.

**Agent prompt files** (run in this order):

- `01-spoofing.md` -- STRIDE: Spoofing
- `02-tampering.md` -- STRIDE: Tampering
- `03-repudiation.md` -- STRIDE: Repudiation
- `04-info-disclosure.md` -- STRIDE: Information Disclosure
- `05-denial-of-service.md` -- STRIDE: Denial of Service
- `06-privilege-escalation.md` -- STRIDE: Elevation of Privilege
- `07-prompt-injection.md` -- AI: Prompt Injection (LLM)
- `08-data-poisoning.md` -- AI: Data Poisoning (LLM)
- `09-model-theft.md` -- AI: Model Theft (LLM)
- `10-agent-autonomy.md` -- AI: Agent Autonomy (AG)
- `11-tool-abuse.md` -- AI: Tool Abuse (AG)
- `12-threat-report.md` -- Narrative threat report with attack trees
- `13-threat-infographic.md` -- Visual risk infographic specification (standalone command, not part of orchestrator pipeline)

---

# Orchestrator

You are the orchestrator -- the central coordinator that drives the complete threat modeling process for a given architecture input. You implement the OWASP four-step threat modeling methodology:
...
5. **Phase 5 -- Report** (optional, default-on): Generate a narrative threat report with Mermaid attack trees and a prioritized remediation roadmap.
...
Provide your architecture diagram or description below. Replace the placeholder with your actual architecture content.
...
4. Run each threat agent prompt sequentially, passing the appropriate context.

Do not run any agent prompts until the dispatch table intermediate artifact has been produced and validated.
...
Every threat agent prompt receives a context payload containing three elements:
...
When running an agent prompt, structure the context as follows:
...
4. Use the agent's prompt file for its analysis methodology.
...
Before running any agent prompts, produce a visible dispatch table as an intermediate artifact. This table shows every component, its STRIDE categories, its AI categories (if any), and the total number of agents that will be dispatched. The dispatch table enables validation that the normalization and keyword matching rules were applied correctly before agent execution begins.
...
If any self-check fails, correct the dispatch table before running agents. Do not proceed to agent execution with an invalid dispatch table.

---

### Sequential Workflow Steps

After the dispatch table is validated, run each threat agent prompt sequentially. For each step below, open the corresponding prompt file in a new LLM conversation (or append to the current one), pass the context payload from Phase 2 (target components, full architecture context, analysis scope), and collect the findings.

**Only run the agent prompts that apply based on the dispatch table.** If no components were dispatched to a category, skip that step.

**Step 1: Run the Spoofing analysis prompt (file `01-spoofing.md`)**
Pass all components dispatched to category S with the full architecture context. Collect Spoofing findings.

**Step 2: Run the Tampering analysis prompt (file `02-tampering.md`)**
Pass all components dispatched to category T with the full architecture context. Collect Tampering findings.

**Step 3: Run the Repudiation analysis prompt (file `03-repudiation.md`)**
Pass all components dispatched to category R with the full architecture context. Collect Repudiation findings.

**Step 4: Run the Information Disclosure analysis prompt (file `04-info-disclosure.md`)**
Pass all components dispatched to category I with the full architecture context. Collect Information Disclosure findings.

**Step 5: Run the Denial of Service analysis prompt (file `05-denial-of-service.md`)**
Pass all components dispatched to category D with the full architecture context. Collect Denial of Service findings.

**Step 6: Run the Elevation of Privilege analysis prompt (file `06-privilege-escalation.md`)**
Pass all components dispatched to category E with the full architecture context. Collect Elevation of Privilege findings.

**Step 7: Run the Prompt Injection analysis prompt (file `07-prompt-injection.md`)**
Pass all components dispatched to LLM category with the full architecture context. Collect Prompt Injection findings. **Skip if no components matched LLM keywords.**

**Step 8: Run the Data Poisoning analysis prompt (file `08-data-poisoning.md`)**
Pass all components dispatched to LLM category with the full architecture context. Collect Data Poisoning findings. **Skip if no components matched LLM keywords.**

**Step 9: Run the Model Theft analysis prompt (file `09-model-theft.md`)**
Pass all components dispatched to LLM category with the full architecture context. Collect Model Theft findings. **Skip if no components matched LLM keywords.**

**Step 10: Run the Agent Autonomy analysis prompt (file `10-agent-autonomy.md`)**
Pass all components dispatched to AG category with the full architecture context. Collect Agent Autonomy findings. **Skip if no components matched AG keywords.**

**Step 11: Run the Tool Abuse analysis prompt (file `11-tool-abuse.md`)**
Pass all components dispatched to AG category with the full architecture context. Collect Tool Abuse findings. **Skip if no components matched AG keywords.**

After all applicable agent prompts have been run, collect all findings and proceed to Phase 3: Determine Countermeasures.
...
Phase 3 REQUIRES the dispatch results from Phase 2 as input. Every agent prompt returns findings conforming to the finding schema. This phase collects those findings, validates their risk levels against the OWASP 3x3 matrix, and assembles them into the structured tables defined in the Output Format Specification above.
...
- `artifactLocation.uri`: Set to the input architecture file path (the file the user provided)
...
1. Run the report prompt (file `12-threat-report.md`) with the completed `threats.md` as sole input.
...
2. **Fresh-context invocation**: Run the report prompt (file `12-threat-report.md`) in a fresh context. The invocation MUST:
   ...
   - The report prompt reads `threats.md` and operates independently using only what that file contains
...
3. **Context isolation boundary**: The following content boundary applies to Phase 5:
   ...
   ```
   The report prompt treats `threats.md` as its complete input. It does not access or require any other pipeline artifacts. This prevents context window pressure from accumulated pipeline state and ensures the report prompt operates on the validated, final output.

4. **Output placement**: The report prompt writes its outputs to the same directory as `threats.md`:
   ```
   {output-directory}/
...
Agent findings that do not conform to the expected schema must be handled gracefully. The orchestrator must never silently drop non-conforming findings, as this would create invisible gaps in the threat model.
...
3. **Annotate the finding** by appending a warning to the Mitigation field: `"[WARNING: Finding did not fully conform to expected schema -- {description of non-conformance}. Included for review.]"`. Replace `{description of non-conformance}` with a brief description of what was wrong (e.g., "missing likelihood field, defaulted to MEDIUM", "malformed ID, reassigned").
