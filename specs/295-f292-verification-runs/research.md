...
## Plan-Stage Decisions (Phase 0 — resolves all open seams; no NEEDS CLARIFICATION remain)

### D-A — T017 comparison-SARIF path (resolves PRD OQ-5 / Architect M-a)

- **Decision**: Primary path stays single-agent (`tachi-output-integrity` direct dispatch). The comparison SARIF for the fresh side is produced by a **one-off assembler step over the assembled findings**: parse the agent's YAML finding blocks (schema v1.9) and emit a SARIF whose results carry exactly the filter-contract surface — `partialFingerprints["findingId/v1"]`, `message.text`, and the drift-bucket properties — reusing `scripts/sarif_common.py::build_sarif_envelope` (and `generate-threats-sarif.py::build_result` where the dict-shape mapping is clean) via the `importlib` precedent (`tests/scripts/test_affected_assets_wiring.py:520-536`). The assembler lives at `specs/295-f292-verification-runs/tools/` — verification tooling, NOT product code (`scripts/` untouched beyond FR-014). Raw agent output is committed verbatim alongside. The corrected filter then applies **uniformly to both sides**; envelope-level byte deltas get a standing attribution class ("assembler-tier envelope vs LLM-authored anchor"); gate-field deltas remain hard failures.
- **Rationale**: Anchor-side SARIF result shape inspected live (2026-07-03): OI results carry findingId only in `partialFingerprints`; sink/flow identity is embedded in `message.text` lead clauses; no dedicated component property. Emission-level gate (D-1) needs {count, findingId set, sink/flow identity} — the assembler preserves exactly these; fabricating a synthetic `threats.md` to feed the full generator would invent a pipeline stage that exists nowhere in production. Single-agent primary preserves the PRD's overflow-avoidance rationale (no 14-agent dispatch).
- **Alternatives considered**: (a) Scoped-full run's native SARIF as primary — same-tier comparison (cleanest byte attribution) but reinstates the orchestrator dispatch the primary path exists to avoid; retained as the **fallback**, whose native `threats.sarif` feeds the same corrected filter. (b) Tuple-set normalization without any SARIF — cheapest, but breaks spec FR-002's "corrected filter applies to the comparison artifact" uniformity and weakens AC-1a's non-empty-extraction guard.
- **Gate-field extraction semantics**: count + findingId set are pure-jq deterministic; **sink/flow identity** is compared as *identifiers* (named DFD components + quoted flow names), assembled into a per-finding identity table in the verification record with both sides' verbatim text committed — message *prose* stays in the attributable-drift bucket per D-1 (two LLM sessions phrase the same emission differently; identification ≠ wording).

### D-B — US-3 CI wiring mechanism (resolves spec FR-016 mechanism choice)

- **Decision**: **Dedicated single-runner workflow** (working name `tachi-sarif-regen.yml`) per the `tachi-catalog-drift.yml` precedent: single `ubuntu-latest` (OS-independent assertion), `permissions: contents: read`, PR + `push:[main]` triggers via one shared paths anchor, direct pytest invocation of the new module.
- **Rationale**: The assertion is OS-independent and sub-second — joining `tachi-pytest.yml` would run it on a 2-OS bash-compat matrix built for init.sh hardening (wrong concern, inflated wall time) and would make every `examples/multi-tenant-rag-app/*` edit trigger the full init matrix. Repo precedent favors purpose-named single-concern workflows (catalog-drift, maestro-coverage, citation-linkrot, permissions-verify).
- **Alternatives considered**: (a) join `tachi-pytest.yml` 15-module list — rejected (concern mismatch, matrix cost, paths-anchor blast radius); (b) extend `tachi-catalog-drift.yml` — rejected (couples taxonomy-fingerprint triggers with example-baseline triggers; muddies both signals for ~40 saved lines).
- **Paths anchor (lock-step per ADR-039 D-6)**: the new test module, `scripts/generate-threats-sarif.py`, `scripts/sarif_common.py`, `examples/multi-tenant-rag-app/threats.md`, `examples/multi-tenant-rag-app/threats.sarif`, and the workflow file itself — all in BOTH the paths filter AND the invocation, one commit.

### D-C — FR-014 covering-assertion location (P0, independent of US-3's P1 fate)

- **Decision**: The URI-derivation covering assertion lands in **`tests/scripts/test_affected_assets_wiring.py`** (already in the 15-module CI gate; already `importlib`-loads the generator), and `scripts/generate-threats-sarif.py` is added to `tachi-pytest.yml`'s `&hardening_paths` anchor in the same commit.
- **Rationale**: FR-014 is P0; its assertion cannot live in the P1 US-3 module (which defers on T026 gate failure per FR-018). The gated suite now asserts on the generator, so the generator must be a trigger path — that IS the lock-step principle (KB Entry 3 / ADR-039 D-6). Generator edits are rare; the marginal matrix cost is accepted. Post-US-3, the dedicated workflow additionally guards the generator end-to-end.
- **Alternatives considered**: new P0 test module — rejected (rung 2: an existing gated module already loads the generator; one assertion does not justify a module + wiring).

### D-D — T026 run mechanics & committed layout

- **Decision**: Run `tachi.threat-model examples/multi-tenant-rag-app/architecture.md --output-dir examples/multi-tenant-rag-app/test-output/` (gitignored by `.gitignore:210` — conventional, local-only). Copy the 4 LLM-authored artifacts (`threats.md`, `threat-report.md`, `risk-scores.md`, `risk-scores.sarif`) from the timestamped run dir to the example **root** (README pairing-contract layout; `maestro-reference` precedent); script-generate `threats.sarif` from the copied `threats.md` via the URI-corrected generator; commit exactly those 5. Run provenance (command, run-dir listing, populator confirmation) goes in the T026 verification record; `attack-trees/`/`attack-chains` remain uncommitted (AC-2b set is exact).
- **Rationale**: default output dir `docs/security/` is gitignored; run dirs are timestamped so artifacts never land at the example root directly; committing the run dir would exceed AC-2b's exact set.
- **Alternatives considered**: root-level `sample-report/` subdir layout (agentic-app precedent) — rejected: the README pairing contract (`examples/README.md:9`) and the two root-level precedents match adopter browsing expectations for a *listed* example; sample-report/ exists for full-suite showcases.

### D-E — Pre-state suite set (FR-019)

- **Decision**: Record literal totals for: `test_backward_compatibility.py` (full module, incl. slow PDF byte-identity), the maestro-coverage pair (`test_maestro_coverage_invariant.py`, `test_maestro_cross_surface_consistency.py`), `test_catalog_drift_guard.py`, and `test_affected_assets_wiring.py` — plus a grep sweep for `examples/**` corpus-globbing or count-pinned assertions across `tests/`. Committed as `specs/295-f292-verification-runs/test-results/pre-state.md` BEFORE any artifact/enabler commit.
- **Rationale**: exactly the suites whose surfaces F-295 touches (generator, example corpus, SARIF pair); KB Entry 15 inherited-vs-own attribution discipline.
