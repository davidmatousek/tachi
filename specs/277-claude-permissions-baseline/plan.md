...
**Target Platform**: GitHub-hosted repo (`github.com/pratik-saptarshi/tachi-rust`); read by Claude Code at session startup for permission resolution; read by humans (CLAUDE_PERMISSIONS.md + ADR-041) for policy review; consumed via `git pull` / `make update`.
...
1. **Pre-commit at /aod.build implementation time**:
   ...
   6. AC-7 subdomain-matching manual probe: `WebFetch(api.github.com/repos/pratik-saptarshi/tachi-rust)` surfaces a prompt (per Issues #15260 / #11972 / #1217 expected outcome)
