...
- [ ] T001 Confirm current branch is `250-adversarial-unit-extraction-hotfix` and draft PR #253 is open: run `git branch --show-current` (expect `250-adversarial-unit-extraction-hotfix`) and `gh pr view 253 --json isDraft,title,headRefName` (expect `isDraft=true`, `title="fix(250): extract adversarial unit tests — eliminate cold-cache CI flake"`, `headRefName="250-adversarial-unit-extraction-hotfix"`).
- [ ] T002 [P] Confirm dev workstation bash version supports the deliberate-fault matrix (T013): run `bash --version`. If version is bash 5.x (Linux dev) or bash 3.2 (macOS dev), record it in `.aod/results/deliberate-fault-matrix-250.md` header. Bash 5.x is required for T013 to exercise the `patsub_replacement` failure mode.
- [ ] T003 [P] Confirm pre-hot-fix `tests/scripts/test_init_sh_adversarial.py` line count matches the contract: `wc -l tests/scripts/test_init_sh_adversarial.py` MUST report **288** lines (the architect baseline cited 289 — both endpoints inclusive vs exclusive accounting agree). Confirm the deletion-block anchors: `sed -n '41p;162p;165p;227p;229p;288p' tests/scripts/test_init_sh_adversarial.py` (expect lines 41 and 162 to bracket the block to delete; lines 165, 227, 229, 288 to bracket the preserved blocks).
- [ ] T004 [P] Confirm load-bearing shim is present at the cited offset: `grep -n "shopt -u patsub_replacement 2>/dev/null" .aod/scripts/bash/template-substitute.sh` MUST report a match at line 64. This shim's removal is the FR-010 / SC-006 [MANUAL-ONLY] regression-detection invariant; if the line moves, update the test in T007.
...
- [ ] T020 (post-squash-merge) Within ~30s of squash-merging PR #253 to `main`, confirm a release-please patch-bump PR opens automatically: `gh pr list --state open --search "release-please" --limit 3`. If empty, push an empty release-marker commit per `.claude/rules/git-workflow.md` belt-and-suspenders rule: `git commit --allow-empty -m "fix(250): extract adversarial unit tests — release marker" && git push origin main`. **FR-018 closure**.
