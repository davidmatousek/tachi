...
- [ ] T016 [P] [US3] Write + run specs/185-cwe-catalog-expansion/scripts/name_diff.py — all-(40 or add-set) name comparison of inserted cwe.yaml records vs cwec_v4.20.xml harvest → 0 mismatches (R7 gate; CWE-1039's v4.17 rename is the sentinel case); evidence in specs/185-cwe-catalog-expansion/test-results/name-diff.md
- [ ] T017 [P] [US3] Review sweep (code-reviewer): no-excluded-edge-returns check (the 1 other-drift + 20 non-CWE + 25 dedupe edges remain absent from crosswalk.yaml); stale-count grep sweep (`53 record|53-record|578` in docs/ + schemas/ outside specs/, update any found); evidence in specs/185-cwe-catalog-expansion/test-results/review-sweep.md
