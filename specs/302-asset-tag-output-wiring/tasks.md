...
- [ ] T012 [US1] Update the production LLM authoring contract in `.claude/agents/tachi/risk-scorer.md` SARIF section: risk-scorer emits `risk-scores.sarif` `result.properties.affected_assets` (snake_case) copied verbatim from the `threats.md` block; **§3.5 scoring + 9.2 ceiling UNCHANGED** (frozen). Depends on T004.
