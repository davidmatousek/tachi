...
- [X] T001 Record LITERAL pre-state pytest totals: run `python3 -m pytest tests/scripts/test_extract_report_data.py tests/scripts/test_extractor_contract_fixes.py -q` and save exact pass/fail/total counts + command output to specs/217-detect-images-duplicate-cleanup/pre-state.md (KB Entry 15 discipline)
- [X] T002 Extend `run_extract` in tests/scripts/test_extract_report_data.py with optional `extra_args=None` parameter appended to the subprocess argv (contracts/cli-contract.md §4); all existing call sites unchanged; re-run the module to confirm totals still match T001
