...
**Testing**: Two regression gates. (1) **Content-level diff** of `threats.md` on 6 example architectures (Phase 1a gate, Phase 1b gate, Phase 3 final) — checks finding count per category ±2, severity distribution ±1 per level, zero dropped findings. (2) **Byte-deterministic PDF re-baseline** on 5 examples with `SOURCE_DATE_EPOCH=1700000000` per ADR-021 (same mechanism as Feature 136). No new test framework; existing `tests/scripts/` pytest baseline stays as-is.
