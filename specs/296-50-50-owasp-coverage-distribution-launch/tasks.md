...
- Local-only (gitignored): `_internal/strategy/BLP-04-adoption-push.md` (canonical per `_internal/CLAUDE.md`; consolidated 2026-05-29 — F-296 plan's `docs/product/_internal/` path was wrong)
- Project memory: `/Users/david/.claude/projects/-Users-david-Projects-tachi/memory/feedback_aod_deliver_release_gate.md`
- Out-of-tree external: LinkedIn (maintainer account), `davidmatousek/Cybersecurity-Content` (article PR), `davidmatousek/davidmatousek` (profile refresh PR), `github.com/pratik-saptarshi/tachi-rust/discussions/179` (closing comment)
...
- [X] T010 [P] [US1] [MANUAL-ONLY] **PASS — see notes/in-tree-merge-closeout.md (`\*` renders as literal `*` per GitHub /markdown API).** Architect L-1 render-check: push the README hero commit to the feature branch and render the file on github.com (PR #297 file diff preview, or branch-specific URL `https://github.com/pratik-saptarshi/tachi-rust/blob/296-50-50-owasp-coverage-distribution-launch/README.md`). Confirm `\*` footnote-marker renders as a literal asterisk (not the escape sequence). If rendering shows `\*` literally, swap to unicode dagger (`†`) or superscript `(¹)` and update the footer text accordingly. Re-verify on github.com.
...
- [X] T012 [P] [US1] **PASS — see notes/in-tree-merge-closeout.md (5 rows + exact footnote + 6 URLs 200 OK + anchor link).** Verify hero block rendering on github.com PR #297 file preview: (a) 5 framework rows visible in first viewport; (b) all 5 anchor links resolve (200 OK); (c) Web/API combined-slot footnote text matches "OWASP Web Top 10:2021 (A01–A10) + OWASP API Security Top 10:2023 (API1–API10) — 20 items, 20/20"; (d) `docs/standards/OWASP_COVERAGE.md` link resolves to the file authored in T005.
...
- [X] T015 [US3] [MANUAL-ONLY] **SUPERSEDED — standalone article not pursued (maintainer direction 2026-05-29); the write-up lives in-repo (`notes/cybersec-article-draft.md`, public on main), and the LinkedIn post + `OWASP_COVERAGE.md` carry distribution. See Issue #296 close.** Open a PR against `davidmatousek/Cybersecurity-Content` with the article body from T013/T014. Title format: `feat: 50/50 OWASP coverage in tachi — what it means and how to verify it` (or similar matching the Cybersec Content repo's conventional commit style if any). Record the PR URL in a local note `specs/296-50-50-owasp-coverage-distribution-launch/notes/cybersec-article-pr-url.txt` for use at T028 (Issue #296 close). **Hold ≥24 hours before self-merge** per NFR-008 (re-read with fresh eyes).
- [X] T016 [US3] **SUPERSEDED — no standalone article to merge (see T015).** After ≥24-hour hold, re-read the PR with fresh eyes and run NFR-008 checklist once more. Self-merge the PR. Update the URL file (T015 output) with the merged-state URL (or canonical article URL on the Cybersec Content site).
...
- [X] T018 [US2] [MANUAL-ONLY] **Done — published 2026-05-29, verified public (SC-002): linkedin.com/posts/…activity-7466143603090788352. URL at notes/linkedin-post-url.txt. Standalone post (no article link; repo + OWASP_COVERAGE.md in T+60 comment).** Publish the LinkedIn post from T017 on the maintainer's LinkedIn account (web or mobile). Capture the post URL immediately after publication. Record in `specs/296-50-50-owasp-coverage-distribution-launch/notes/linkedin-post-url.txt` for use at T028. **Publication MUST happen AFTER T016 (Cybersec article merged) per Q2 lean** — verify the article URL in the post body resolves (200 OK) at the moment of publication.
- [X] T019 [P] [US2] [MANUAL-ONLY] **Done — format recorded at notes/linkedin-post-url.txt (native-content-first; links in T+60 first-comment, not body; statement-style close). Reach metrics deferred to F-3.** Spec US2 AC-3 algorithmic-reach validation: this AC is non-automatable; maintainer judgment governs. Note in the URL file (T018 output) the chosen format (native-content-first; article URL location — body vs first-comment) and the publication time window. Reach metrics observation is deferred to F-3 (BLP-04 Wave 2 adoption signal capture); F-1 ships the post and records the URL.
...
- [X] T020 [US4] [MANUAL-ONLY] Open a PR against `davidmatousek/davidmatousek` repo updating the profile README per plan.md §Wave 3 template. Tachi flagship surface with: (a) 50/50 OWASP coverage tagline, (b) STRIDE+AI threat modeling harness one-line description, (c) link to tachi repo. AOD-Kit secondary position (visible, not removed). Per PRD Q7 lean = minimal — flagship project table only; NO "Now" section. Profile footer wording at author discretion within ADR-044 dual-frame alignment (Architect plan-layer L-3). Record PR URL in `specs/296-50-50-owasp-coverage-distribution-launch/notes/profile-pr-url.txt`. **Hold ≥24 hours before self-merge** mirror of NFR-008 discipline (R4 mitigation). **Done**: PR #1 opened at https://github.com/davidmatousek/davidmatousek/pull/1 (branch `refresh-tachi-flagship-50-50` → master). Delta = flagship block (first viewport, AC-1) with 50/50 tagline (AC-2) + STRIDE+AI+MAESTRO one-liner (AC-3) + repo link; Security Tooling row refreshed; AOD-Kit untouched/secondary (AC-4); footer unchanged (author discretion). Draft + PR package at `notes/profile-readme-draft.md`. URL recorded at `notes/profile-pr-url.txt`. ≥24h hold + self-merge = T021 (pending, MANUAL-ONLY). **Scope reframe (maintainer-directed, post-open)**: PR #1 grew from the minimal Q7 delta into a full CISO/VP-audience repositioning (8 commits): lead "Cybersecurity Leader · App/AI/Cloud · Risk & Compliance"; tachi + AOD co-flagships under "What I build" (OWASP 50/50 tagline + STRIDE+AI+MAESTRO one-liner = AC-2/AC-3 ✓; AOD visible/elevated = AC-4 ✓; tachi named in first-viewport hook = AC-1 in spirit, detailed flagship block mid-page). Retired "LinkedIn Top Voice" credential removed; AOD launch (agentic-oriented-development.com) wired as canonical link. Exceeds PRD Q7 "minimal" lean per explicit maintainer direction (profile now serves David's career positioning with tachi/AOD as proof-of-building). Canonical content = PR #1; `notes/profile-readme-draft.md` updated to final state.
- [X] T021 [US4] [MANUAL-ONLY] After ≥24-hour hold + before/after rendering review on github.com (R4 mitigation), self-merge the profile refresh PR. Update the URL file (T020 output) with merged-state URL. **Done**: PR #1 squash-merged 2026-05-28 (merge commit 13c03b5) on maintainer authorization ("merge it"); profile live on `davidmatousek/davidmatousek` master. Cooling-off hold waived by maintainer after extensive in-session review (8 commits workshopped). Feature branch deleted. `notes/profile-pr-url.txt` updated to merged state.
...
- [X] T023 [US5] [MANUAL-ONLY] **Done — de-personalized closing comment posted 2026-05-29 via GraphQL addDiscussionComment: discussioncomment-17116904. No @-mention (maintainer direction; US-5 AC-2/3/5 deviation noted in Issue #296 close).** On Day 5 AM (≤2026-06-04 per FR-005 7-day SLA + SC-005), publish the draft from T022 to Discussion #179 via the GitHub Discussions UI (or `gh discussion comment 179 --body-file ...` if available). Verify @armorer-labs receives a notification (verifiable via @-mention rendering in the published comment per US5 AC-5).
- [X] T024 [US5] [MANUAL-ONLY] **Done — Discussion #179 closed RESOLVED 2026-05-29 via GraphQL closeDiscussion. Close URL at notes/discussion-179-close-url.txt.** Close Discussion #179 via `gh discussion close 179 --reason resolved` (or via the Discussions UI). Mark discussion status as "shipped" / "resolved". Record the discussion URL with closing-comment anchor in `specs/296-50-50-owasp-coverage-distribution-launch/notes/discussion-179-close-url.txt` for use at T028.
