...
- [X] T020 [US1] After the substitution loop in `scripts/init.sh`, add a residual scan: `aod_template_assert_no_residual "$file"` per file (re-walk the same `find` filter or store the file list from T019). On any non-zero return, print `[init] FATAL: residual placeholder in <file>; aborting.` and exit 1 (FR-004) — **CODE LANDS in 4ace34a but FUNCTIONALLY BLOCKED on Defect 1 (residual-scan regex over-scope; halts on ~110 legitimate non-canonical tokens in stack scaffolds, brand templates, devops docs, and doc examples). See tasks-runlog.txt §T021 for full disposition.**
- [ ] T021 [US1] Stream 1 Day 1 post-swap benchmark (NFR-004): time `./scripts/init.sh` on the canonical fixture (matching T008 inputs); calculate delta vs T008 baseline (`(post - pre) / pre * 100%`). Apply NFR-004 escalation: ≤5% holds NFR-004 at 10% (record numbers); 5–50% loosens NFR-004 to 25% with rationale (record in ADR-038 §Consequences); >50% escalate to PM for re-scope before merge — **BLOCKED at +658% delta (50.687s vs 6.690s baseline). PM re-scope + Architect residual-scan-regex review required. See tasks-runlog.txt §T021.**

**Checkpoint**: ⚠ Stream 1 Wave 3 G3 gate BLOCKED — T020 functional defect + T021 NFR-004 breach. PM + Architect disposition required before T014 / Streams 2-4 can advance.
