...
- [X] T048 [MANUAL] Verify release-please opens release PR within ~30s post-merge: `gh pr list --state open --search "release-please" --limit 3`; if empty, push empty release-marker commit (FR-010 R12 fallback): `git commit --allow-empty -m "feat(248): substitution surface hardening — release marker" && git push origin main` — **CLOSED 2026-05-04: release-please PR #251 opened automatically post-merge; title "chore(main): release 4.28.0"; head ref release-please--branches--main; no release-marker fallback needed; FR-010 R12 verified.**
