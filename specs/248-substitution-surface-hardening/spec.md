...
**Independent Test**: Run `init.sh` with `PROJECT_NAME=AT&T` (and other adversarial values) on a controlled fixture. Grep the substituted tree for the literal value; confirm count matches baseline; confirm no `{{PROJECT_NAME}}` remnants and no metacharacter-interpretation artifacts (`AT{{PROJECT_NAME}}T`, truncation at `&`, etc.).

**Acceptance Scenarios**:

1. **Given** a fresh checkout with `PROJECT_NAME=AT&T`, **When** `init.sh` runs to completion, **Then** every occurrence of `{{PROJECT_NAME}}` in personalized files contains the literal string `AT&T` and `grep -r "AT&T" <substituted-tree>` matches the expected count from the recorded baseline fixture byte-for-byte.
...
**Outcome (provisional, to be reconfirmed during Stream 1 Day 1 build)**: Codebase research found **no wired consumer** within the project tree. The file is documentation-only (the comment block at line 18 reads *"Replace {{PROJECT_PATH}} with the absolute path to your project root"*). Claude Code's actual MCP config lives at `~/.config/claude-code/`, not in the project tree. **Default disposition is therefore Option (b): remove the file.** If a wired consumer surfaces during Stream 1 Day 1 build (e.g., a `make` target that copies the file, or an agent persona that reads it from the project tree), the disposition flips to Option (a) and `PROJECT_PATH` is added to the canonical placeholder array per FR-007's fallback clause.
