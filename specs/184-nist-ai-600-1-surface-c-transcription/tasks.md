...
- [ ] T001 Verify baseline integrity suite green: `/usr/bin/python3 -m pytest tests/schemas/test_taxonomy_integrity.py -v` → 5/5 (~1s). Record output in `specs/184-nist-ai-600-1-surface-c-transcription/test-results/w0-baseline.txt`. **STOP-gate**: abort and escalate if not 5/5.
