...
| **W4 Polish** `[P]` | ADR-027 Revision History entry + Decision 3 annotation + `docs/architecture/README.md` blurb (one commit); taxonomy README §3.8 + §1 snippet (8 stems) + §2 amendment + composition (~L243) + L13/L20 counts; stale-count sweep (inventory surfaces only; TWO expected `542` historical survivors — crosswalk L14 lineage + README ~L193 F-182 narrative); CHANGELOG `feat(184)` | senior-backend-engineer (architect reviews ADR entry text) | Sweep greps clean on inventory surfaces; zero diffs on exempt surfaces; ADR ratified-text byte-unchanged above the annotation |
