...
- [ ] T009 Deliver gate S-1 (constitution VII / git-workflow; depends on T006, T007, T008): **do NOT `git push` the branch or open/ready the PR until T006 is green locally** (the FR-006 `push:[main]` gate would otherwise redden `main`). Then merge to `main` through **devops** with a Conventional-Commit PR title — `fix(338): restore F-248/F-256 substitution hardening` (release-triggering).
- [ ] T010 Post-merge release verification (depends on T009): confirm a release-please PR opened within ~30s of the squash-merge (`gh pr list --state open --search "release-please" --limit 3`); if empty, push an empty `fix(338): restore F-248/F-256 hardening — release marker` commit (AOD deliver-release gate).
