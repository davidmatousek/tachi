...
- Out-of-tree: GitHub repo settings (`Settings → Code security and analysis → Advanced Security → Private vulnerability reporting`); GitHub Security tab (`https://github.com/pratik-saptarshi/tachi-rust/security`)
...
**Independent Test**: Open `/Users/david/Projects/tachi/SECURITY.md` post-T004. Scroll to the "Reporting a Vulnerability" section. Confirm: (a) the *Report a vulnerability* button is referenced as the primary instruction with a link to `https://github.com/pratik-saptarshi/tachi-rust/security`; (b) the fallback URL `https://github.com/pratik-saptarshi/tachi-rust/security/advisories/new` is present; (c) the explicit "do not open a public Issue" prohibition is stated; (d) a "What to include" subsection lists description / steps to reproduce / affected components / potential impact; (e) the R-2 maintainer footer references the PVR-toggle dependency verbatim per PRD §R-2.
...
- [X] T008 [P] [US2] [MANUAL-ONLY] Verify the *Report a vulnerability* button is visible on `https://github.com/pratik-saptarshi/tachi-rust/security` as a primary call-to-action per FR-011. Reload the page if necessary; the button should appear within seconds of T006 completion. **Result 2026-05-08**: Maintainer confirmed *Report a vulnerability* button visible on Security tab in authenticated browser session. FR-011 satisfied.
- [X] T009 [P] [US2] [MANUAL-ONLY] Smoke-test the advisory submission URL per FR-012: open `https://github.com/pratik-saptarshi/tachi-rust/security/advisories/new` in the authenticated browser session. Confirm the form loads (no 404, no permission error). Do NOT submit the form. Close the tab. **Result 2026-05-08**: Maintainer confirmed advisory submission form loads cleanly (no 404, no permission error); form not submitted per spec. FR-012 satisfied.
...
EOF
)` per FR-014. Confirm the PR title begins with `feat(272):` to ensure release-please will trigger on squash-merge. **Result 2026-05-08**: Branch `272-security-md-disclosure` pushed to origin (new remote branch, upstream tracking set). Draft PR **#273** created at https://github.com/pratik-saptarshi/tachi-rust/pull/273 with title `feat(272): SECURITY.md and private disclosure channel` (verified via `gh pr view 273 --json title,isDraft` → title prefix `feat(272):` ✅, isDraft `true` ✅). FR-014 first-surface enforcement satisfied per `.claude/rules/git-workflow.md` Conventional-Commit-PR-Titles. PR# **273** captured for downstream T017 (PR-description-edit) + T018 (gh pr ready) + T019 (squash-merge) + T020 (release-please verify).

### Toggle evidence + PR-ready

- [X] T017 Edit the draft PR description to include the toggle-verification evidence captured in T007 (screenshot + plain-text "Toggle confirmed ON at …" string) per AC-6 + D-5. Use `gh pr edit <PR#> --body-file <(...)` to update the body. Confirm both screenshot reference and plain-text confirmation are present. **Result 2026-05-08**: PR #273 body updated via `gh pr edit 273 --body <heredoc>`. Plain-text confirmation present and verified by grep: `**Toggle confirmed ON at 15:34 UTC 2026-05-08 via repo settings UI.**` Per Session 4 user decision (architect concern N-1 resolution), screenshot attachment was deliberately omitted in favor of the AC-6 + D-5 plain-text fallback path; rationale: the durable string is reproducible by maintainer re-visiting repo settings and is supplemented by the public-surface "Report a vulnerability" button-visibility test in the Test plan checklist. PR title still `feat(272): SECURITY.md and private disclosure channel`; isDraft still `true` (T018 will flip it).
- [X] T018 Mark the PR as ready for review: `gh pr ready <PR#>`. Confirm via `gh pr view <PR#> --json isDraft` that `isDraft` is now `false`. (Per `.claude/rules/git-workflow.md`, the squash-merge into main is the release-please trigger; ready-for-review unblocks the merge.) **Result 2026-05-08**: `gh pr ready 273` reported "Pull request pratik-saptarshi/tachi-rust#273 is marked as 'ready for review'"; `gh pr view 273 --json isDraft --jq .isDraft` returned `false`. PR-ready unblock confirmed; squash-merge gate (T019) cleared.

### Squash-merge + post-merge verification

- [X] T019 Squash-merge the PR: `gh pr merge <PR#> --squash`. Verify the merge succeeded and that the squash-commit subject on `main` retains the `feat(272):` prefix via `git checkout main && git pull && git log --oneline -1`. (Per F-212 incident: a non-conventional commit subject silently skips release-please.) **Result 2026-05-08**: `gh pr merge 273 --squash` succeeded. `git checkout main && git pull --ff-only` advanced main from `1853883` to `7b1cc53`. `git log --oneline -1` shows `7b1cc53 feat(272): SECURITY.md and private disclosure channel (#273)` — `feat(272):` prefix preserved on main; release-please trigger intact (verified at T020). 15-file fast-forward includes SECURITY.md, README.md, CHANGELOG.md, plus all specs/272/ and PRD artifacts. F-212 recovery preconditions NOT met (subject is conventional); skipped per spec.
- [X] T020 Verify a release-please PR opens within ~30 seconds of the squash-merge per FR-014: `sleep 30 && gh pr list --state open --search "release-please" --limit 3`. **If empty**, push an empty release-marker commit per the F-212 recovery flow: `git commit --allow-empty -m "feat(272): SECURITY.md and private disclosure channel — release marker" && git push origin main`. Re-check `gh pr list` until a release-please PR appears. **Result 2026-05-08**: Release-please PR **#274** (`chore(main): release 4.33.0`, branch `release-please--branches--main`) detected at 0s of the post-merge poll window — release-please reacted instantly when squash-commit `7b1cc53` landed on main (the `git pull --ff-only` log already showed `release-please--branches--main` force-pushed by the GitHub Actions release-please runner before the poll started). FR-014 first-surface enforcement satisfied; F-212 recovery flow NOT triggered (no empty release-marker needed). N-3 deliberate-execution checkpoint honored: the polling loop ran via `run_in_background` per the system's leading-sleep block (alternative to the literal `sleep 30 && gh pr list` from the team-lead's M-2 minor concern); architect's N-3 intent ("do not skip even if release-please appears auto-functional") is satisfied because the verification was explicitly executed and recorded — not assumed.
- [X] T021 Run `/security` re-scan post-merge. Confirm `.aod/results/security-scan.md` records `TACHI-VULN-05abc41ad4cc → REMEDIATED` per FR-013. Confirm no new HIGH or MEDIUM findings; LOW or INFO side-effect findings are acceptable if they document a separate concern. Capture the re-scan output for the /aod.deliver retrospective. **Result 2026-05-08**: `/security` skill invoked from main HEAD `7b1cc53`; the squash-merge introduced changes only to documentation files (SECURITY.md, README.md, CHANGELOG.md, specs/, docs/product/) — no code-extension files and no dependency manifests, so SAST and SCA both reach the Step 1a/1b zero-file SKIPPED paths and the clean-scan path (Step 5c) yields **scan_status = PASSED**. Per the Step 6c lifecycle rule, `TACHI-VULN-05abc41ad4cc` (previously DETECTED at 2026-05-02T10:51:56Z, scan_id `5cd62414...`) is absent from the current scan findings and is therefore appended to `.security/vulnerabilities.jsonl` as **REMEDIATED** at scan_id `5345e7a2-5944-486c-b6e7-8e57e238261e` ts `2026-05-08T17:32:45Z`. Other 10 historical vuln_ids are already REMEDIATED from F-1/F-2 closures and are not re-emitted (idempotent semantics). Artifacts written: `.security/scan-log.jsonl` (PASSED entry, chain_hash `337228...`), `.security/vulnerabilities.jsonl` (REMEDIATED event), `.security/reports/7b1cc53e6f57.sarif` (empty results SARIF 2.1.0 with `executionSuccessful: true` + `remediated_vuln_ids: ["TACHI-VULN-05abc41ad4cc"]` invocation property), `specs/272-security-md-disclosure/security-scan.md` (FR-026 report), and `.aod/results/security-scan.md` (task-literal mirror per architect N-4 path). FR-013 satisfied: explicit REMEDIATED state recorded; zero new HIGH/MEDIUM/LOW/INFO findings introduced. Gate C (post-merge clean) PASSED.

### Follow-up Issues (AC-13 + AC-14)

- [X] T022 [P] File AC-13 follow-up Issue: `gh issue create --title "[chore] PVR-toggle posture probe (post-F-3 follow-up)" --body "Periodic check that the GitHub Private Vulnerability Reporting toggle remains ON. Concrete probe: scrape https://api.github.com/repos/pratik-saptarshi/tachi-rust and assert security_and_analysis.private_vulnerability_reporting.status == \"enabled\". Origin: F-3 (PRD AC-13, deferred at /aod.spec). Captures toggle-state regressions before researchers experience the broken channel. ICE rough estimate: I:6 C:8 E:9."`. Capture the resulting Issue number for the /aod.deliver retrospective. **Result 2026-05-08**: Issue **#275** filed (https://github.com/pratik-saptarshi/tachi-rust/issues/275); AC-13 posture-probe follow-up captured for /aod.deliver retrospective.
- [X] T023 [P] File AC-14 follow-up chore Issue: `gh issue create --title "[chore] release-please manifest-vs-tag discrepancy investigation (post-F-3 follow-up)" --body "Observed at PRD draft 2026-05-08 and re-confirmed at /aod.project-plan: .release-please-manifest.json reports 4.31.0 while latest git tag is v4.32.0 with no open release-please PR. Investigate whether the v4.32.0 release-please cycle updated the tag without the manifest, or whether the manifest got rolled back post-merge. Origin: F-3 (PRD AC-14, deferred at /aod.spec). Not blocking F-3 because SECURITY.md text references the latest tag, not the manifest."`. Capture the resulting Issue number for the /aod.deliver retrospective. **Result 2026-05-08**: Issue **#276** filed (https://github.com/pratik-saptarshi/tachi-rust/issues/276); AC-14 release-please manifest-vs-tag discrepancy follow-up captured for /aod.deliver retrospective. Of incidental note: as of T020 release-please PR #274 (4.33.0) opening, the manifest-vs-tag delta may be naturally resolved on next release-please cycle; #276 will track resolution either way.
...
# Tab 1: https://github.com/pratik-saptarshi/tachi-rust/security  (T008 button visibility)
# Tab 2: https://github.com/pratik-saptarshi/tachi-rust/security/advisories/new  (T009 form load)
