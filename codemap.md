...
| `docs/` | Public project documentation. Roadmap and product planning documents live under `docs/roadmap/`; testing status lives under `docs/testing/`; archived security-review guidance for retired FastAPI scaffolds lives in `docs/security/`; the root `README.md` now treats the old FastAPI packs as archived legacy guidance, live examples use Rust/Tauri or shell tooling instead of Python pretty-printers, and the pre-commit install guidance now avoids Python-specific installation wording. |
...
| RT-014 | Retire Python packaging, pytest-only guidance, and FastAPI stack scaffolds after parity is complete; the security-review FastAPI scaffold note is archived guidance only, the active architecture system-design README now points at Rust CLI extractor commands, and the pre-commit guidance no longer recommends Python-package installation wording. |
