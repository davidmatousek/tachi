...
| RT-014 | Retire Python packaging, pytest-only guidance, and FastAPI stack scaffolds after parity is complete; the security-review FastAPI scaffold note is archived guidance only, the active architecture system-design README now points at Rust CLI extractor commands, and the pre-commit / CLAUDE organization / permissions guidance no longer recommends Python-package installation or pytest wording. |
