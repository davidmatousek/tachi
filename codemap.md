...
| Integration | Rust integration tests under `crates/*/tests` and `src-tauri/tests`; current audit shows 58 Rust integration modules after retiring the defaults-env init, adversarial init, template git clone timeout, executive-architecture infographic, attack-chain, MAESTRO pattern-classification, init precommit matrix, PDF page-positioning, backward-compatibility, human-trust-exploitation, extractor contract fixes, coverage-attestation tiers, init constitution, tool-abuse enrichment, and pattern-synthesis pytests while the init-substitution E2E boundary is now Rust-owned. |
...
| Coverage | `make llvm-cov` is the release-quality local gate. Current validated baseline: 86.42% regions / 86.78% lines. Current audit: 64 active modules, 58 Rust integration modules, 0 Python unit modules, 0 Python smoke modules, 5 support/regression modules. |
