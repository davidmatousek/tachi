...
- **Pre-commit secret-scanning** → [docs/standards/PRECOMMIT_HOOKS.md](docs/standards/PRECOMMIT_HOOKS.md) (gitleaks default-secure hook; existing adopters opt-in via `pre-commit install`; starter config: [`.gitleaks.toml.adopter-template`](.gitleaks.toml.adopter-template), see PRECOMMIT_HOOKS.md §9.5)
- **Real-world usage** → [In the Wild](https://github.com/davidmatousek/tachi/discussions/categories/in-the-wild) or the [adopters index](docs/adopters/) — tell me how you're using tachi, anonymized is fine
...
```bash
~/Projects/tachi/scripts/install.sh --version v4.46.0 # x-release-please-version
```
...
To install a specific version: `install.sh --version v4.46.0` <!-- x-release-please-version -->
