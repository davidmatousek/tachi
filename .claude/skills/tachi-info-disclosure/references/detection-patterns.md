...
**Cross-reference to OWASP API10:2023 (Unsafe Consumption of APIs)**:

The SSRF surface in this category intersects with OWASP API Security Top 10 2023 API10:2023 (Unsafe Consumption of APIs) when the consumer makes outbound calls to a "trusted" upstream third-party API whose redirect target is attacker-influenced — i.e., the upstream API issues a redirect that points to cloud metadata or to an internal service the consumer would not normally reach, and the consumer follows the redirect without re-applying egress policy. The Primary Pattern Category for the consumed-API-response trust failure (the inverse flow — upstream-to-consumer payload trust feeding injection-prone sinks) is `tachi-tampering` Pattern Category 9 (Injection Attacks Beyond SQL — extended for consumed-API-response trust per the F-241 Stream 2 disposition). This Pattern Category 7 covers the SSRF dimension when the redirect-target trust failure is the architectural-tell.
...
## Pattern Category 10: API Surface Inventory and Documentation Exposure (OWASP API9:2023)

OWASP API9:2023 (Improper Inventory Management) names the disclosure of the API surface itself — the set of reachable endpoints, hosts, environment tiers, and version-tagged routes — as a distinct confidentiality class from per-endpoint data leakage (Excessive Data in Responses), from runtime metadata exfiltration (Pattern Category 7 SSRF), and from server-side debug-output leakage (Pattern Category 8). The architectural-tell here is leakage of architectural metadata about the API itself: undocumented dev/staging/legacy endpoints reachable from production network paths, unauthenticated machine-readable API descriptions (`/openapi.json`, `/swagger-ui`, GraphQL introspection) retained in production, multiple version-tagged route prefixes co-resident on the same host with no deprecation enforcement, and dev/staging/sandbox hostnames resolvable from public DNS without IP allowlisting. The disclosure chain is two-stage and indirect: leaked surface-inventory information enables later targeted exploitation against the weakest endpoint in the inventory — typically a deprecated v1 route with weaker rate limiting, a beta endpoint with a missing authorization check, or a staging environment with reused production credentials. Because the leakage is about architecture rather than application data, traditional per-endpoint data-classification controls do not detect or prevent it; the mitigation surface is inventory governance and environment-tier isolation, not field-level redaction.

**Indicators**:

- Production environment retains unauthenticated machine-readable API descriptions — `/swagger-ui`, `/swagger-ui.html`, `/api-docs`, `/openapi.json`, `/openapi.yaml`, `/.well-known/openapi`, `/redoc` — reachable without authentication or IP allowlisting from the external trust zone
- GraphQL introspection enabled in production — `__schema { types { name fields { name type { name } } } }` resolvable on the production endpoint without authentication, exposing the full type system, query/mutation surface, and field-level type signatures
- Multiple version-tagged route prefixes co-resident on the same production host — `/api/v1/*`, `/api/v2/*`, `/api/v3/*` all reachable concurrently with no declared deprecation timeline; deprecated `/v0` retained but never removed; beta-tagged routes (`/api/v3-beta/*`, `/api/preview/*`) reachable without a feature-flag gate at the gateway tier
- Dev/staging/sandbox hostnames resolvable from public DNS or routable from the production network — `dev-api.example.com`, `staging.example.com`, `sandbox-api.example.com` with public A/AAAA records and reachable ALB/ingress endpoints, no IP allowlist restricting access to corporate or VPN CIDRs
- No declared API inventory artifact maintained in source control — no `inventory.yaml`, no central API catalog, no OpenAPI registry under version control, no service-mesh service-discovery export tagging hosts/routes by version and environment-tier
- Old API versions or environment metadata advertised in HTTP response headers — `Allow: GET, POST` revealing supported methods on undocumented routes; `Server: nginx/1.18.0`, `X-Powered-By: Express`, `X-API-Version: v1.4.7-deprecated` echoing internal versioning detail; `X-Environment: staging` leaking environment tier
- Beta or feature-flag-gated endpoints reachable without the feature-flag claim — gateway-tier flag check absent; the application-layer flag check is the only gate, leaving the endpoint discoverable via direct route enumeration even when the application denies the response
- Container or Kubernetes annotations leak environment names through application-exposed surfaces — `/info`, `/actuator/info`, `/metrics`, `/healthz` echoing `environment: dev`, `deployment.kubernetes.io/revision`, namespace identifiers, or pod labels into the external response payload
- Cross-environment OAuth issuer or session-credential reuse — staging and production share the same OAuth issuer or session-token format, so a token harvested from a discoverable staging endpoint authenticates against production
...
- CWE-200: Exposure of Sensitive Information to an Unauthorized Actor: https://cwe.mitre.org/data/definitions/200.html
- OWASP API Security Top 10 2023 — Project Home: https://owasp.org/API-Security/
- NIST SP 800-204: Security Strategies for Microservices-based Application Systems: https://csrc.nist.gov/publications/detail/sp/800-204/final

**Example**: A regional fintech platform operates a payment API at `api.fintech.example` with a current `v3` route prefix that enforces device-attestation, per-tenant token-budget rate limiting, and step-up authentication on transfer endpoints. During a v2-to-v3 migration the team retained the deprecated `/api/v1/transfer` and `/api/v2/transfer` endpoints "for backwards compatibility with one legacy partner" — but never removed them after that partner finished migrating eight months earlier. The production host also retains an unauthenticated `/swagger-ui` page (originally enabled for developer onboarding, never gated behind auth in production) that lists every route across v1, v2, and v3 with parameter schemas, response shapes, and example payloads. An attacker performs reconnaissance: a single GET to `api.fintech.example/swagger-ui` returns the full inventory; the attacker discovers `/api/v1/transfer` accepts a transfer with no device-attestation header and a per-IP rate limit of 1000 requests/minute (compared to the v3 endpoint's per-tenant 60-requests/hour cap with mandatory device-attestation). Using a stolen session token harvested from a separate phishing campaign, the attacker drives transfer fraud through the v1 endpoint at a rate that the v3 anti-abuse stack would have throttled in seconds; the bypass is invisible to the v3 anomaly detection pipeline because the fraud traffic never reaches the v3 surface. A parallel disclosure path: the same platform's `staging-api.fintech.example` resolves to a public IP (no IP allowlist on the staging ALB) and serves the same OpenAPI spec as production; an attacker harvesting the staging spec discovers an unreleased beta `/api/v4-beta/transfer-bulk` endpoint with weaker authorization gating, schema-maps the new `bulk_recipients[]` field for offline analysis, and pivots to attacking the staging-tier OAuth issuer (which shares its key material with production) for a credential-replay attack. Both attack chains depend on the same root architectural defect: the API surface inventory itself is leaked, even though no individual endpoint leaks application data in its response payload.

**Mitigation**:

- Maintain a single source-of-truth API inventory artifact — `inventory.yaml` (or equivalent OpenAPI-registry manifest) versioned in git, regenerated from declared OpenAPI specs on every deploy, with each entry tagged by host, environment-tier, version, owner, and deprecation-status; treat the inventory as the authoritative gate for what is allowed to be reachable in production, and use it as the source for gateway-tier route enforcement
- Enforce environment-tier hostname and network separation — production, staging, dev, and sandbox each on distinct hostnames with private DNS for non-production tiers (resolvable only from corporate VPN or VPC-internal resolvers), IP allowlisting on staging/dev/sandbox ALBs to corporate or VPN CIDRs, and no shared OAuth issuer or session-credential format across tiers
- Gate `/swagger-ui`, `/openapi.json`, `/api-docs`, `/redoc`, and equivalent machine-readable API descriptions behind authentication or internal-network-only ingress in production — a 401 or 403 (or absent route returning 404) on the external trust zone, with the documentation mirror published only on a developer-portal endpoint that requires the developer's own authenticated session
- Disable GraphQL introspection in production — set `GraphQLSchema.introspection = false` (or framework equivalent), or gate introspection behind an `Authorization: Bearer <internal-tools-token>` claim; mirror with a similar gate on `/api/explorer`, `/playground`, `/graphiql` developer surfaces
- Declare and enforce a deprecation cycle for old API versions — every version-tagged route has a documented sunset date and a `Sunset:` and `Deprecation:` HTTP response header on every response served from the deprecated route during the deprecation window; gateway-tier removal automation removes the route from the production gateway on the sunset date with no manual step required
- Strip versioning, framework-version, and environment-name disclosure from response headers — remove `Server`, `X-Powered-By`, `X-AspNet-Version`, `X-API-Version`, `X-Environment`, and `X-Kubernetes-*` headers at the gateway tier; configure framework defaults to omit identifying banners
- Run automated API discovery against production CIDR weekly — tools such as Burp Suite Enterprise, OWASP ZAP, `kiterunner`, `feroxbuster`, or `gobuster` (with rate limits respecting production traffic quality) enumerate routes against the public ingress and diff the discovered set against the declared `inventory.yaml`; alert on any route reachable in production that is not in the inventory, treating the diff as a deploy-gate failure
- Document the API surface inventory choices in threat-modeling artifacts and as a deploy-gate compliance artifact — include the inventory diff result, the environment-separation network diagram, and the deprecation timeline as evidence in compliance audits
...
- OWASP API Security Top 10 2023 — API9: Improper Inventory Management: https://owasp.org/API-Security/editions/2023/en/0xa9-improper-inventory-management/
- OWASP API Security Top 10 2023 — Project Home: https://owasp.org/API-Security/
- OWASP Error Handling Cheat Sheet: https://cheatsheetseries.owasp.org/cheatsheets/Error_Handling_Cheat_Sheet.html
- OWASP SSRF Prevention Cheat Sheet: https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html
