...
4. Assign `likelihood` and `impact` using OWASP factors (attacker skill, availability of injection tools, input validation coverage; data integrity loss, financial impact, downstream system corruption), then compute `risk_level` via the OWASP 3×3 matrix in `severity-bands-shared.md`.
