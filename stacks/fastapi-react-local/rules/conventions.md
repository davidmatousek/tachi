# Archived Conventions — Python FastAPI + React (Local / SQLite)
...
- Use Alembic with async `run_sync()` and `render_as_batch=True` for migrations.
- Enable WAL mode and foreign keys via SQLite PRAGMAs on every connection.
- Use `sqlite+aiosqlite:///` as the connection string scheme.
- Use `JSON` column type (stored as text in SQLite) instead of native `ARRAY`.
...
- Use native `ARRAY` column types (not supported by SQLite).
- Use `ALTER COLUMN` or `DROP COLUMN` in Alembic without `render_as_batch=True`.
