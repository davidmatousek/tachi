# Senior Backend Engineer — Python FastAPI + React Supplement

Archived legacy pack; retained for historical reference only.

## Stack Context

FastAPI 0.115+ async REST API, SQLAlchemy 2.0 async ORM with asyncpg driver, Pydantic v2 for request/response schemas and settings, Alembic for async migrations, uv package manager, Ruff for linting/formatting, pytest + pytest-asyncio for testing, httpx AsyncClient for integration tests.
...
- ALWAYS use `postgresql+asyncpg://` as the connection string scheme -- never `postgresql://` or `postgresql+psycopg2://`
...
- ALWAYS use `NullPool` for test engines to prevent connection pool interference between function-scoped fixtures
...
- Database session: `app/db/session.py` -- `create_async_engine` + `async_sessionmaker` + `get_db` generator
- Dependencies: `app/api/deps.py` -- `get_db()`, `get_current_user()`, shared `Depends()` callables
- Configuration: `app/config.py` -- single `Settings(BaseSettings)` class, never scattered `os.getenv()` calls
- Migrations: `alembic/versions/` -- generate with `uv run alembic revision --autogenerate -m "{description}"`
- Tests: `tests/conftest.py` (async fixtures), `tests/api/` (integration), `tests/services/` (unit)
