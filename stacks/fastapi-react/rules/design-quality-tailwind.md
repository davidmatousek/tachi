...
This stack uses Vite for bundling. The Tailwind entry point is `frontend/src/app.css`.
...
**Vite note**: Vite processes CSS through PostCSS. Tailwind v4 uses the `@tailwindcss/vite` plugin — ensure it is registered in `vite.config.ts`:
...
Use CSS custom properties for stagger delays. Define in your `@theme` or as utility layer:

```css
/* frontend/src/app.css — inside @theme or as utility */
@layer utilities {
...
**Vite note**: Vite supports CSS `@layer` out of the box. No extra configuration needed for custom utility layers.
...
Font loading in a Vite + React project uses `@font-face` in CSS or a package like `fontsource`:
...
```css
/* frontend/src/app.css — before @theme */
...
**Vite note**: Fontsource packages are self-hosted — no external requests to Google Fonts. Vite tree-shakes unused weights automatically.
...
**Vite note**: CSS custom properties are resolved at runtime by the browser, not at build time. Vite does not transform `var()` references — this is expected and correct.
...
For loading state, use React state to conditionally swap content:
...
**Vite note**: Vite's HMR preserves component state during development, so you can iterate on loading states without resetting the form. Use React DevTools to toggle `isLoading` prop for testing.
