...
The bash-pattern workflows (manifest-coverage, extract-coverage, stack-contract) share the same bash:3.2 Docker pattern, SHA-pinned checkout action, `contents: read` permissions, and cancel-in-progress concurrency. See `docs/devops/CI_CD_GUIDE.md` for the reusable pattern including the `apk add git` + `safe.directory` setup. The host-runner workflows (`tachi-pytest.yml`, `tachi-mmdc-preflight.yml`, `gitleaks.yml`, `tachi-maestro-coverage.yml`) follow a different shape — direct host execution with path-filtered triggers — because their workloads (Python+bash subprocess, mmdc Node binary preflight, native gitleaks binary, Python markdown-glob assertion) do not benefit from container isolation. A third shape is the **scheduled monitor** (`tachi-citation-linkrot.yml`, F-183): cron + `workflow_dispatch` only (NO `push`/`pull_request`), single ubuntu runner, `contents: read` + `issues: write`, ambient `GITHUB_TOKEN` (no PAT/secret). It is a **non-gating monitor** — it exits green even when link-rot is found and must never be a required check (operators read its tracking issue, not the run color).
...
- [CI/CD Guide](CI_CD_GUIDE.md) — includes manifest-coverage + extract-coverage + stack-contract workflows, tachi-pytest (F-248 + F-256 + F-282 + F-302) + tachi-mmdc-preflight (F145) + gitleaks (F-282 / F-5) + tachi-maestro-coverage (F-315 / #313) host-runner workflows, tachi-citation-linkrot (F-183, scheduled monitor) workflow, bash:3.2 Docker pattern, BATS harness, update-script env vars
...
- **No CI workflow changes**: F-4 touched zero `.github/workflows/*.yml` files. The `.claude/settings.json` file is read by the Claude Code harness at agent invocation time on the developer's local machine — it never executes inside `manifest-coverage`, `extract-coverage`, `stack-contract`, `tachi-pytest`, or `tachi-mmdc-preflight` workflows. CI continues to apply unchanged.
...
- **`tachi-pytest.yml` path-filter delta**: the F-248 + F-256 substitution-hardening workflow gained one new test module entry — `tests/scripts/test_init_precommit_matrix.py` — added to BOTH the `paths:` trigger set AND the `python -m pytest ...` invocation in lock-step (per the F-256 lock-step pattern documented at CI_CD_GUIDE.md L181-L218). Edits to F-5's init.sh prompt block now correctly fire the pytest matrix on both `macos-latest` (bash 3.2.57 floor) and `ubuntu-latest` (bash 5.x reference).
...
- **`scripts/init.sh` opt-in prompt block** (DELTA): added an opt-in pre-commit hook prompt at the secret-scanning phase with default `Y` in TTY contexts. Two new flag overrides: `--precommit` forces install regardless of TTY/answer; `--no-precommit` forces skip. Non-TTY contexts (`</dev/null`, CI, scripted invocation) skip the prompt silently. The prompt phase also enforces a **`pre-commit --version >= 3.5.0` floor check** (Architect CONCERN-3) — if installed but below the floor, init.sh logs a WARN with upgrade guidance and continues; if not installed, init.sh logs a WARN with install guidance (`pip install pre-commit` / `brew install pre-commit`) and continues. Init.sh **never aborts** on pre-commit framework absence or version mismatch — the F-5 hook is opt-in defense-in-depth, not a hard prerequisite.
...
- **New tachi adopters** (running `scripts/init.sh` for the first time): the prompt at init time is default-Y; pressing `<Enter>` accepts. If `pre-commit` is not installed system-wide, init.sh logs a WARN and continues — install via `pip install pre-commit` (or `brew install pre-commit`) and run `pre-commit install` from the repo root to enable the hook post-init.
- **Existing tachi adopters** (already running tachi via `make update` / `git pull`): F-5 is opt-in. To enable the hook, run from the repo root: `pip install pre-commit` (if not already installed; or `brew install pre-commit`) followed by `pre-commit install`. Pulling F-5 does NOT auto-install the hook (FR-010).
...
## Feature 302 Additions (2026-06-01)

The **Asset-Tag Output Wiring** feature (F-260b, PR #303, feature branch `302-asset-tag-output-wiring` squash-merged on 2026-06-01, merge commit `3d3d29f`) wired @north-echo's asset-sensitivity tags (merged as a half-integrated prototype in v4.31.0 via PR #262) end-to-end through tachi's output stack — a `finding.yaml` schema field, deterministic populator, `threats.md` serialization, and SARIF emission in both emitters. The only DevOps-relevant surface is **CI**: F-302 is a scanner output-wiring feature with **no staging/production deployment surface, no new runtime services, no Docker services, no new monitoring, and no adopter-facing environment variables**. The hook/test execution is CI-only (`pull_request`) plus local pytest.

- **`tachi-pytest.yml` path-filter + pytest-invocation lock-step delta (T020)**: the F-248 + F-256 + F-282 cross-platform pytest matrix gained the previously-unwired asset-tag suite. Two test modules were added to BOTH the `paths:` trigger set AND the `python -m pytest ...` invocation in lock-step (per the F-256 lock-step pattern documented at `docs/devops/CI_CD_GUIDE.md` → "Tachi Pytest Workflow"): `tests/scripts/test_asset_sensitivity_tags.py` (26-case asset-tag suite) and `tests/scripts/test_affected_assets_wiring.py` (populator + SARIF `affected_assets` wiring, 35 cases). Combined, both F-260b suites run 61/61 green on the `macos-latest` (bash 3.2.57 floor) + `ubuntu-latest` (bash 5.x) matrix.
- **Three source surfaces added to `paths:` only (trigger-only)**: `scripts/populate-affected-assets.py` (deterministic populator — value authority), `scripts/sarif_common.py` (`affected_assets` extractor — verification tier), and `schemas/finding.yaml` (`affected_assets` schema contract, bumped 1.8 → 1.9). These are production/schema files exercised by the suites, not test entry points, so they belong in the trigger list only — not the pytest invocation. This ensures edits to F-260b code fire the matrix even when no test file changed.
- **No CI workflow changes beyond `tachi-pytest.yml`**: `manifest-coverage.yml` (F129), `extract-coverage.yml` (F128), `stack-contract.yml` (F130), `gitleaks.yml` (F-282), and `tachi-mmdc-preflight.yml` (F145) all continue to apply unchanged. No new workflows were added.
- **No new env vars, no secret changes, no deployment-environment changes**: F-302 introduces zero environment variables of any kind (cross-referenced in `docs/devops/environment-variables.md`). Staging and Production READMEs are intentionally unchanged. `docs/architecture/04_deployment_environments/*.md` are intentionally unchanged.
- **Frozen-constraint gate (SC-011)**: `git diff main -- schemas/risk-scoring.yaml scripts/tachi_parsers.py` shows zero change to `VALID_ASSET_TAGS` (6-value enum), the `modifier_ceiling: 9.2`, the modifier-after-clamp ordering, or `risk-scoring.yaml` `schema_version` (stays 1.1). F-302 is wiring, not re-tuning.

**Adopter action**: none at the DevOps layer. Adopters who run `make update` receive the new schema field + output serialization via the standard F-129 manifest-driven sync; existing CI invocations are unaffected. PRD: `docs/product/02_PRD/302-asset-tag-output-wiring-2026-05-30.md`. Spec + tasks: `specs/302-asset-tag-output-wiring/`. ADR: `docs/architecture/02_ADRs/ADR-046-asset-tag-output-wiring.md`. Workflow walkthrough: `docs/devops/CI_CD_GUIDE.md` → "Tachi Pytest Workflow (F-248 + F-250 + F-256 + F-282 + F-302)". Credit: the asset-sensitivity tag prototype was authored by @north-echo (PR #262).

---

## Feature 183 Additions (2026-06-15)

The **Citation-URL Link-Rot Monitoring** feature (F-183, PR #330, feature branch `183-citation-url-link-rot-monitoring` squash-merged on 2026-06-15, merge commit `0a33d70`) shipped a new **scheduled GitHub Actions workflow** that watches the health of every citation URL referenced by the tachi crosswalk catalog (the framework-mapping output authority built across BLP-05 Wave 2). F-183 closes BLP-05 Wave 3 and ships in release **v4.44.0**. It is a **CI monitor** with no staging/production deployment surface, no new runtime services, no new secrets, and no new environment variables:

- **New scheduled workflow**: `.github/workflows/tachi-citation-linkrot.yml` runs `scripts/check-citation-urls.py` on a weekly cron (`17 9 * * 1` = Mondays 09:17 UTC) plus manual `workflow_dispatch`. The dispatch input `inject_sentinel_rot` (boolean, default `false`) injects one pre-classified synthetic rot finding to exercise the GitHub-issue lifecycle with no network calls. The job probes citation URLs, detects link rot, and reconciles a **single self-healing GitHub tracking issue** (open / update / comment / close as rot appears and clears). Full walkthrough: `docs/devops/CI_CD_GUIDE.md` → "Tachi Citation Link-Rot Monitor (F-183 / #183)".
- **Monitor, not a gate (operator-critical)**: the checker exits **`0` even when link-rot IS found** — a found-rot run is still a **GREEN** run. Only a genuine infra failure (script crash / `gh` auth failure) reddens it. **Operators must read the tracking issue for rot state, NOT the run color**, and this workflow must **NEVER be wired as a required check**. A green checkmark means "the probe completed," not "no rot."
- **Scheduled-only / never-on-PR (NFR-001)**: the workflow declares **NO `on.pull_request` and NO `on.push`** — only the cron + `workflow_dispatch`. Citation reachability depends on the live, non-deterministic internet; wiring it to PR/push would let a flaky external host redden an unrelated merge (a determinism-boundary violation). Do not add a PR trigger or path filter when extending it.
- **Least privilege (NFR-005)**: permissions are exactly `contents: read` (checkout) + `issues: write` (manage the tracking issue) — no `pull-requests`, no `contents: write`. `gh` authenticates with the **ambient `GITHUB_TOKEN`** via the `GH_TOKEN` env var (`secrets.GITHUB_TOKEN`) — **no PAT, no new repository secret required**.
- **Ledger caching**: a combined `actions/cache@v4` step (restore + save in one) persists `linkrot-ledger.json` run-over-run. The primary key embeds `${{ github.run_id }}` (always misses on restore so the post-job save always fires and the ledger accumulates); `restore-keys: linkrot-ledger-v1-` resolves the newest prior ledger on the way in. `actions/cache` is a new pattern for this repo — restore-key accumulation should be validated on the first two dispatch runs.
- **Runtime**: `ubuntu-latest` (single OS — the probe is network I/O + markdown/YAML parsing, OS-independent), Python 3.11, `pip install pyyaml>=6` only (no `requests`, no other new runtime dependency — standard library + pyyaml).
- **Manual run command**: `gh workflow run tachi-citation-linkrot.yml -f inject_sentinel_rot=true|false`.
- **No env var changes, no secret changes, no deployment-environment changes**: F-183 introduces zero environment variables (cross-referenced in `docs/devops/environment-variables.md`). Staging and Production READMEs are intentionally unchanged at the deployment layer; the scheduled-job note is recorded in `docs/devops/03_Production/README.md` → "Scheduled / Non-Deploy Workflows". `docs/architecture/04_deployment_environments/*.md` are unchanged.

**Adopter action**: none at the DevOps layer. Adopters who run `make update` receive the new workflow + checker via the standard F-129 manifest-driven sync; the workflow only fires on the weekly cron or explicit manual dispatch and never on their PRs/pushes. Do NOT add it to branch protection. PRD/spec: `specs/183-citation-url-link-rot-monitoring/spec.md` (NFR-001 scheduled-only, NFR-005 least privilege). Workflow contract: `specs/183-citation-url-link-rot-monitoring/contracts/workflow.md`. Workflow walkthrough: `docs/devops/CI_CD_GUIDE.md` → "Tachi Citation Link-Rot Monitor (F-183 / #183)". Checker: `scripts/check-citation-urls.py`.
