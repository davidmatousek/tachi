...
- **Rust-only CI dependencies**: the workflow installs the stable Rust toolchain and does not need Python package installation at runtime. Cross-suite Python dependency notes remain only for archival fixtures and historical docs.
...
- **Performance characteristic on macOS**: the matrix's macOS leg is still the slowest runner, but the Rust suite removes the Python `pytest` dependency and keeps the same bash 3.2.57 compatibility gate. Rust test timing remains bounded by the same shell hardening surfaces, and the historical 300s/360s pytest timeout notes are retained only for archive context.
...
- **`tachi-pytest.yml` path-filter delta**: the Rust init matrix gained Rust test module entries — `crates/tachi-shell/tests/init_substitution.rs`, `crates/tachi-shell/tests/init_constitution.rs`, `crates/tachi-shell/tests/init_defaults_env.rs`, `crates/tachi-shell/tests/init_precommit_matrix.rs`, `crates/tachi-shell/tests/init_timing_trace.rs`, `crates/tachi-shell/tests/init_trace_summary.rs`, and `crates/tachi-core/tests/active_docs_runtime_guidance.rs` — added to BOTH the `paths:` trigger set AND the `cargo test` invocation in lock-step. Edits to F-5's init.sh prompt block now correctly fire the Rust matrix on both `macos-latest` (bash 3.2.57 floor) and `ubuntu-latest` (bash 5.x reference).
