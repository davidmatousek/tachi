...
### Default-rule coverage catalog

The per-rule rationale catalog above documents tachi's *additive* `.gitleaks.toml` entries. This catalog answers a different question: for each canonical credential pattern, which **upstream default gitleaks rule** actually fires? A SecOps reviewer auditing coverage needs both catalogs — this one confirms the default ruleset behaves as claimed, without reverse-engineering rule names from gitleaks source.

| Canonical pattern | Example fixture | Active default RuleID | Covered |
|---|---|---|---|
| GitHub PAT (`ghp_*`) | `staged-credential/github-pat.txt` | `github-pat` | ✅ |
| AWS access key (`AKIA*`) | `staged-credential/aws-access-key.txt` | `aws-access-token` | ✅ |
| OpenAI key (`sk-*`) | `staged-credential/openai-key.txt` | `generic-api-key` | ✅ |
| Anthropic key (`sk-ant-*`) | `staged-credential/anthropic-key.txt` | `anthropic-api-key` | ✅ |
...
**Known gap: generic high-entropy hex strings.** Hex-alphabet secrets (e.g., a raw 32-character hex token with no distinguishing prefix) fall below the default `generic-api-key` rule's entropy/pattern threshold and are **not** caught by any default rule. This is a real, disclosed gap — not a claimed capability. It is tracked as enhancement [#348](https://github.com/davidmatousek/tachi/issues/348). Do not read the §3 summary line's "generic high-entropy strings" phrasing as a guarantee against this specific gap; the catalog above is the authoritative coverage statement.
...
### 9.5 Starter template: `.gitleaks.toml.adopter-template`

Tachi ships a repo-root starter file, [`.gitleaks.toml.adopter-template`](../../.gitleaks.toml.adopter-template), for adopters who want a guided starting point instead of hand-writing §9.1 additions from scratch. Usage:

1. Copy it to `.gitleaks.toml` (fresh adopter) or paste individual commented sections into your existing `.gitleaks.toml` (customizing adopter).
