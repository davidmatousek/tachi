...
| RT-011 | In progress | Continue migrating the remaining pytest coverage after the Rust-native defaults-env init, adversarial init, template git clone timeout, precommit matrix, coverage-attestation in-scope, coverage-attestation report-data guard, coverage-attestation pagination smoke, coverage-percentage computation, MAESTRO pattern-classification rules, and attack-chain extraction migrations. Current audit: 65 active modules, 51 Rust integration modules, 0 Python unit modules, 0 Python smoke modules, 13 support/regression modules. |
