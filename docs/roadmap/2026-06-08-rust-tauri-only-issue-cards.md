...
- **Current audit update**: ML Top 10 coverage bundle enrichment, mobile Top 10 coverage bundle enrichment, LLM10 unbounded consumption coverage, coverage-attestation audit, and init timing trace are now Rust-native. Current audit: 66 active modules, 65 Rust integration modules, 0 Python unit modules, 0 Python smoke modules, 0 support/regression modules.
- **Acceptance**:
...
- **Refinement**: scope placeholder substitution to manifest-backed `personalized` files so unmanifested files are no longer rewritten during init.
