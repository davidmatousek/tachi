...
| RT-012 | In progress | Continue parser/report-data parity after Rust asset tag parsing, report-data image binding emission, infographic output-file handling, coverage-attestation in-scope filtering, coverage-attestation report-data guard, coverage-attestation pagination smoke, coverage-percentage computation, attack-chain extraction, and SARIF runtime retirement; `scripts/extract-report-data.py` and the dead `tests/scripts` helper package are now retired, and the remaining Python runtime dependencies are limited to the parser and stack surfaces still listed in the inventory. |
