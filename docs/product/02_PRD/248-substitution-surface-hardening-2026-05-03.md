...
**Acceptance**: After running `init.sh` with `PROJECT_NAME=AT&T`, every occurrence of `{{PROJECT_NAME}}` in personalization-target files contains the literal string `AT&T` — no `AT{{PROJECT_NAME}}T`, no truncation at `&`. Verified via `grep -r "AT&T" <substituted-tree>` matching the expected count from the baseline fixture.
