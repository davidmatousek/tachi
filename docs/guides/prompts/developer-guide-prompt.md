...
Tachi uses 15 markdown-based agents, coordinated by a central orchestrator:
...
| 14 | `risk-scorer.md` | Risk Scorer | Quantitative 4-dimensional scoring with composite risk calculation |
...
**Post-pipeline commands**: After the 5-phase pipeline completes, run `/tachi.risk-score` for quantitative scoring, `/tachi.compensating-controls` for control analysis, and `/tachi.infographic` for visual risk diagrams (auto-detects richest data source).
...
### 5. `threat-baseball-card-spec.md` — Baseball Card Risk Summary Specification

Structured data for the Baseball Card infographic: donut chart, STRIDE+AI coverage heat map, critical finding cards, architecture overlay strip.

### 6. `threat-baseball-card.jpg` — Baseball Card Infographic Image

Generated via Google Gemini API. Compact risk summary dashboard.

### 7. `threat-system-architecture-spec.md` — System Architecture Specification

Structured data for the System Architecture infographic: trust zones stacked by trust level, components with attack surface badges, data flow arrows colored by severity.

### 8. `threat-system-architecture.jpg` — System Architecture Infographic Image

Generated via Google Gemini API. Annotated architecture diagram with attack surface badges.

### 9. `risk-scores.md` — Quantitative Risk Scores

Generated by `/tachi.risk-score`. Contains:
- Scored threat table with 4-dimensional scores (CVSS, exploitability, scalability, reachability) and composite risk score per finding
- Executive summary with severity distribution and highest-risk component
- Dimensional analysis breakdowns
- Governance fields (owner, SLA, disposition, review date)
- Methodology section documenting the scoring approach

### 10. `risk-scores.sarif` — Scored SARIF Output

SARIF 2.1.0 file with composite scores in `security-severity` property and per-finding scoring dimensions in result property bags. Supersedes `threats.sarif` when available.

### 11. `compensating-controls.md` — Compensating Controls Analysis

Generated by `/tachi.compensating-controls`. Contains:
- Coverage matrix showing control status per threat (Control Found / Partial Control / No Control Found)
- Control details with file:line evidence for all detected controls
- Recommendations sorted by composite score descending
- Residual risk summary comparing inherent vs. residual scores

### 12. `compensating-controls.sarif` — Controls SARIF Output

SARIF 2.1.0 file with residual scores, control properties, and preserved fingerprints from source risk-scores.sarif. Supersedes `risk-scores.sarif` when available.

**Gemini API key requirement**: The developer must have a Gemini API key for infographic image generation. The guide must cover:
...
## Post-Pipeline Enrichment Commands

After the `/tachi.threat-model` command produces the core output suite, three additional commands enrich the results with quantitative scores, control analysis, and visual reports. Each command consumes the previous command's output:
...
/tachi.compensating-controls (risk-scores.md + target codebase)
    ...
    ▼
/tachi.infographic (auto-detects richest: risk-scores.md > threats.md)
    ├── threat-{template}-spec.md, threat-{template}.jpg
```

**When to run each command:**
- `/tachi.risk-score` — **Recommended** after every `/tachi.threat-model` run. Adds quantitative scoring that replaces qualitative severity levels with composite risk scores. Enables governance fields (owner, SLA, disposition).
- `/tachi.compensating-controls` — **Recommended** when you have a codebase to scan. Detects existing security controls, calculates residual risk, and recommends missing controls. Most valuable for established projects with existing security measures.
- `/tachi.infographic` — **Optional** for visual reporting. Produces presentation-ready images for stakeholders. Requires a Gemini API key for image generation; produces spec files without it.

### The `/tachi.risk-score` Command

Adds quantitative risk scoring to threat model findings. Each finding is scored on 4 dimensions and receives a composite risk score on a 0-10 scale.
...
/tachi.risk-score path/to/threat-model-output/

# Custom output directory
/tachi.risk-score path/to/threat-model-output/ --output-dir reports/risk/
```

**Input requirements:**
- `threats.md` (primary, canonical source) or `threats.sarif` (fallback) — must exist in the input directory
- Optional: `architecture.md` in the input directory or its parent — improves reachability scoring
...
**Scoring dimensions (each rated 0-10):**
...
| Exploitability | How easy it is to exploit | Widely known technique, public tools available, low skill required | Novel attack, no public tools, requires expert knowledge |
| Scalability | How many targets can be hit at once | One exploit compromises many users/tenants/systems | Single target, no lateral movement |
| Reachability | How exposed the component is | Internet-facing, crosses trust boundaries, in critical path | Internal-only, no boundary crossings, isolated |

**Composite score:** Weighted formula combining all 4 dimensions — `(CVSS * 0.35) + (Exploitability * 0.30) + (Scalability * 0.15) + (Reachability * 0.20)`. Mapped to severity bands: Critical (>= 9.0), High (7.0-8.9), Medium (4.0-6.9), Low (< 4.0).

**Governance fields** (populated for each finding):

| Field | Purpose |
|-------|---------|
| Owner | Person or team responsible for remediation (default: Unassigned) |
| SLA | Target duration for remediation based on severity (24h, 7d, 30d, 90d) |
...
### The `/tachi.compensating-controls` Command

Scans a target codebase against scored threat findings to detect existing security controls, classify their effectiveness, calculate residual risk, and recommend missing controls.

**Invocation:**

```bash
# Minimal — analyze codebase in current directory
...
**Input requirements:**
- `risk-scores.md` (primary, canonical source) or `risk-scores.sarif` (fallback) — must exist in the input directory
- A target codebase directory with at least one file
- Optional: `architecture.md` — enables architecture-aware component-to-file mapping
...
|--------|--------------|
| Control Found | A security control addressing this threat exists in the codebase with file:line evidence |
| Partial Control | Some mitigation exists but does not fully address the threat |
| No Control Found | No existing control detected — a gap requiring remediation |

**Residual risk calculation:** `Residual Risk = Composite Score x (1 - Reduction Factor)` where Reduction Factor is 0.50 (Control Found), 0.25 (Partial Control), or 0.00 (No Control Found). A finding with composite score 8.5 and a full control has residual risk of 4.3 (`8.5 x (1 - 0.50) = 4.25`, rounded to 4.3).

**Evidence format:** All detected controls include `file:line` references pointing to the exact location in the codebase where the control was found. Example: `src/middleware/auth.ts:42` — JWT validation middleware.

### The `/tachi.infographic` Command (Standalone)

Generates visual threat infographic specifications and presentation-ready images. Auto-detects the richest available data source, preferring `risk-scores.md` over `threats.md`.
...
| `all` | Generate both templates (default) |
| `corporate-white` | Legacy alias, resolves to `baseball-card` |

**Auto-detection behavior:**
1. Scans for `risk-scores.md` in the working directory — if found, uses it as primary source (quantitative composite scores)
2. Falls back to `threats.md` if `risk-scores.md` is absent (qualitative severity counts)
3. When using `risk-scores.md`, requires co-located `threats.md` in the same directory for structural data (project metadata, trust zones, data flows)

**Output artifacts (per template):**
- `threat-{template-name}-spec.md` — Structured infographic specification with 6 sections per `schemas/infographic.yaml`
- `threat-{template-name}.jpg` — Presentation-ready image (only when `GEMINI_API_KEY` is available and API call succeeds)

**Gemini API key:** Required for image generation only. Without it, the command produces specification files but skips image generation. See the Claude Code Installation section for key configuration options.
...
cp ~/Projects/tachi/adapters/claude-code/commands/tachi.threat-model.md .claude/commands/
cp ~/Projects/tachi/adapters/claude-code/commands/tachi.risk-score.md .claude/commands/
cp ~/Projects/tachi/.claude/commands/tachi.compensating-controls.md .claude/commands/
cp ~/Projects/tachi/adapters/claude-code/commands/tachi.infographic.md .claude/commands/
cp ~/Projects/tachi/.claude/commands/tachi.security-report.md .claude/commands/
cp ~/Projects/tachi/.claude/commands/tachi.architecture.md .claude/commands/
...
ls .claude/commands/                   # Should show tachi.threat-model.md, tachi.risk-score.md, tachi.compensating-controls.md, tachi.infographic.md, tachi.architecture.md, tachi.security-report.md
```
...
3. **6 command files** in `.claude/commands/` — `/tachi.threat-model`, `/tachi.risk-score`, `/tachi.compensating-controls`, `/tachi.infographic`, `/tachi.architecture`, `/tachi.security-report`
...
cp adapters/claude-code/commands/tachi.threat-model.md ~/Projects/my-app/.claude/commands/
cp adapters/claude-code/commands/tachi.risk-score.md ~/Projects/my-app/.claude/commands/
cp .claude/commands/tachi.compensating-controls.md ~/Projects/my-app/.claude/commands/
cp adapters/claude-code/commands/tachi.infographic.md ~/Projects/my-app/.claude/commands/
cp .claude/commands/tachi.security-report.md ~/Projects/my-app/.claude/commands/
cp .claude/commands/tachi.architecture.md ~/Projects/my-app/.claude/commands/
```
...
**The workflow:**
1. Install the adapter (copy 15 agent files + commands into your project)
2. Have an architecture description (existing doc, diagram, or write one)
3. Run `/tachi.threat-model` in Claude Code
4. Review the output artifacts

**Invocation — the `/tachi.threat-model` command:**

The adapter includes a `/tachi.threat-model` command that wraps the entire analysis workflow:
...
**Architecture generation — the `/tachi.architecture` command:**

If you don't have an architecture description yet, tachi can generate one from your codebase:

```bash
# Generate architecture description for threat modeling input
/tachi.architecture
```

This produces a structured architecture description suitable as input to `/tachi.threat-model`.
...
7. **Show `/tachi.risk-score` enrichment** — run `/tachi.risk-score` on the OpenClaw threats, show a sample scored output table excerpt with the 4 dimensions and composite score for AG-2 and E-1, explain what the composite score means in practical terms
8. **Show `/tachi.compensating-controls` analysis** — run `/tachi.compensating-controls` on the scored results, show a sample coverage matrix excerpt with Control Found / Partial / No Control Found statuses, show residual risk comparison (inherent vs. residual for AG-2), explain how evidence file:line references point to actual controls
9. **Show `/tachi.infographic` generation** — run `/tachi.infographic` with template selection guidance (baseball-card for executive summary, system-architecture for technical review), describe the spec and image output files
...
3. **Verify**: `ls .claude/agents/tachi/` shows 15 files, `ls .claude/commands/` shows 6 command files (tachi.threat-model.md, tachi.risk-score.md, tachi.compensating-controls.md, tachi.infographic.md, tachi.architecture.md, tachi.security-report.md)
...
5. **Run Your First Analysis**: `/tachi.threat-model` — that is it, one command
...
- Step 11: Running `/tachi.risk-score` — sample scored output with 4 dimensions and composite score, interpretation notes
- Step 12: Running `/tachi.compensating-controls` — sample coverage matrix, residual risk comparison, evidence file:line references
- Step 13: Running `/tachi.infographic` — template selection guidance, sample invocation, output file descriptions
- Include actual output snippets (threats.md excerpts, a sample attack tree, coverage matrix, risk-scores excerpt, controls excerpt)

**Section 4b — Post-Pipeline Enrichment Workflow**
- Pipeline overview diagram showing all 6 commands with inputs, outputs, and dependencies
- When each enrichment command is optional vs. recommended
- Time estimates per command

**Section 4c — Running `/tachi.risk-score`**
...
- Next step: run `/tachi.compensating-controls`

**Section 4d — Running `/tachi.compensating-controls`**
...
- Next step: run `/tachi.infographic`

**Section 4e — Running `/tachi.infographic` Standalone**
...
- Detailed installation walkthrough (agents + `/tachi.threat-model` command)
- Using `/tachi.threat-model` — all flag combinations with examples
...
- Using `/tachi.threat-model .aod/plan.md --output-dir .aod/security/`
...
- Risk-scores.md sections: metadata, scored threat table, risk distribution, dimensional analysis, methodology, governance
- Compensating-controls.md sections: coverage matrix, findings table with status/evidence/recommendations, recommendations sorted by risk, residual risk summary

**Appendix C — Glossary**
- DFD (Data Flow Diagram), STRIDE, OWASP, SARIF, CVE, CWE, CVSS, Trust Boundary, Finding IR, Risk Matrix, Attack Tree, Composite Score, Compensating Control, Residual Risk, Exploitability, Scalability, Reachability — one-line definitions
