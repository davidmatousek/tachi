...
Generated by `/risk-score`. Contains:
...
Generated by `/compensating-controls`. Contains:
...
After the `/threat-model` command produces the core output suite, three additional commands enrich the results with quantitative scores, control analysis, and visual reports. Each command consumes the previous command's output:
...
- `/risk-score` — **Recommended** after every `/threat-model` run. Adds quantitative scoring that replaces qualitative severity levels with composite risk scores. Enables governance fields (owner, SLA, disposition).
- `/compensating-controls` — **Recommended** when you have a codebase to scan. Detects existing security controls, calculates residual risk, and recommends missing controls. Most valuable for established projects with existing security measures.
...
### The `/compensating-controls` Command
...
### The `/infographic` Command (Standalone)
...
7. **Show `/risk-score` enrichment** — run `/risk-score` on the OpenClaw threats, show a sample scored output table excerpt with the 4 dimensions and composite score for AG-2 and E-1, explain what the composite score means in practical terms
8. **Show `/compensating-controls` analysis** — run `/compensating-controls` on the scored results, show a sample coverage matrix excerpt with Control Found / Partial / No Control Found statuses, show residual risk comparison (inherent vs. residual for AG-2), explain how evidence file:line references point to actual controls
9. **Show `/infographic` generation** — run `/infographic` with template selection guidance (baseball-card for executive summary, system-architecture for technical review), describe the spec and image output files
...
3. **Verify**: `ls .claude/agents/tachi/` shows 15 files, `ls .claude/commands/` shows 4 command files (threat-model.md, risk-score.md, compensating-controls.md, infographic.md)
...
5. **Run Your First Analysis**: `/threat-model` — that is it, one command
...
- Step 11: Running `/risk-score` — sample scored output with 4 dimensions and composite score, interpretation notes
- Step 12: Running `/compensating-controls` — sample coverage matrix, residual risk comparison, evidence file:line references
- Step 13: Running `/infographic` — template selection guidance, sample invocation, output file descriptions
...
- Pipeline overview diagram showing all 4 commands with inputs, outputs, and dependencies
- When each enrichment command is optional vs. recommended
- Time estimates per command

**Section 4c — Running `/risk-score`**
...
**Section 4d — Running `/compensating-controls`**
...
**Section 4e — Running `/infographic` Standalone**
