...
**Accepted-commit-SHA**: `8b7c7bf59a6de93a0d3f5016a4395755de19c79e`
...
> Amended at F-184: enum extended to 8 values — see Revision History.
...
**2026-04-17 (Accepted — Feature 180, Wave 5.2 T039 post-merge SHA fill)**: PR #181 squash-merged to `main` at 2026-04-17 21:35:37 UTC with commit SHA `8b7c7bf59a6de93a0d3f5016a4395755de19c79e`. `Accepted-commit-SHA` field on line 5 updated from `<pending-T039-post-merge-fill>` placeholder to the actual squash-merge SHA per FR-041 and the Decision 8 dual-commit governance protocol. **Provisional Accepted-date 2026-04-21 retained** (not corrected to actual merge date 2026-04-17) per T039 guidance: the provisional date reflects the projected T032→T039 deliverable window recorded at Day 4 Wave 4.2 authoring time, and the paired `<pending-T039-post-merge-fill>` placeholder satisfies the "provisional date AND SHA-fill post-merge" pattern; correcting the date would retcon the Day 4 T032 transition record without adding provenance value beyond what the new SHA field now provides. Merge actually occurred 4 days earlier than the provisional target (Day 5 same-day merge vs. projected Day 5 merge window), which confirms the Day 4 Wave 4.2 status-transition cadence was sound. This entry closes the T032 post-merge-fill dependency; all 8 Decisions and the T027 Surface C amendment are carried forward unchanged. No substantive content updates; provenance-strengthening only.

**2026-06-11 (Accepted — Feature 184, W4 T011 Decision 3 extension-governance instrument)**: This entry **is the Decision 3 extension-governance instrument** — Decision 3 closed the 7-value `taxonomy` enum and reserved extension to "a future ADR amending this one"; F-184 ships no new ADR, so the amendment is recorded here, in the ADR being amended, with a one-line additive annotation under the Decision 3 heading and the ratified Decision 1–8 text byte-unchanged. **The `taxonomy` enum is extended from 7 to 8 values**: `nist-ai-600-1` is activated as the 8th value, backed by the new 12-record catalog `schemas/taxonomy/nist-ai-600-1.yaml` (NIST AI 600-1 Generative AI Profile GAI Risks §2.1–§2.12, DOI `https://doi.org/10.6028/NIST.AI.600-1`, ids as YAML-quoted strings in publication order, `cwe_refs: []` throughout — no CWE mapping exists for GAI risks). The filename-stem convention, the closed-enum posture, and the integrity-test fail-on-unknown-value rule from Decision 3 are all preserved. This activation completes the F-A1.1 follow-on scoped by the T027 entry above: **the 15 Surface C Overlap rows of `nist-ai-rmf-mapping.md` are transcribed as 15 `tachi-stride-ai-category → nist-ai-600-1` `primary` edges** (`confidence: high`, citation to the source table), under the **FR-022 direction correction** ratified by PRD-184 — F-180's spec FR-022 pointed Surface C rows at `nist-ai-rmf` Subcategory targets, conflating AI 600-1 GAI Risks with AI RMF 1.0 Subcategories per the ADR-025 three-surface structure; PRD-184 FR-022 corrects the target taxonomy to `nist-ai-600-1`. In the same change-set, the **FR-7 16-edge removal disposition** completes the standing T027 removal directive: the 16 legacy `tachi-stride-ai-category → nist-ai-rmf` drift edges — ordered REMOVED by the T027 entry, left in place as survivors of T029's Option (d) MIX execution — are removed at F-184 (PM ruling Option A per spec FR-004; lineage: T027 directive → T029 Option (d) MIX survivors → removed at F-184). Crosswalk composition moves 542 → 541 `primary` / 37 `related` / 0 `superseded` = 578 edges; the ≥500 primary floor holds (headroom 41). Authority trail: **PRD-184** ([`docs/product/02_PRD/184-nist-ai-600-1-surface-c-transcription-2026-06-10.md`](../../product/02_PRD/184-nist-ai-600-1-surface-c-transcription-2026-06-10.md)), **Issue #184** (BLP-05 Wave 2), and the **Architect sign-off** (tasks-stage APPROVED 2026-06-10, triad block of `specs/184-nist-ai-600-1-surface-c-transcription/tasks.md`; review instrument at `.aod/results/architect.md`). The integrity suite (5 functions) stayed green at every F-184 commit boundary per spec FR-008.
