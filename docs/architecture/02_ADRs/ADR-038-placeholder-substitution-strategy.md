...
`scripts/init.sh:117-159` previously substituted the canonical 12 placeholder set (`{{PROJECT_NAME}}`, `{{PROJECT_DESCRIPTION}}`, `{{GITHUB_ORG}}`, `{{GITHUB_REPO}}`, `{{AI_AGENT}}`, `{{TECH_STACK}}`, `{{TECH_STACK_DATABASE}}`, `{{TECH_STACK_VECTOR}}`, `{{TECH_STACK_AUTH}}`, `{{RATIFICATION_DATE}}`, `{{CURRENT_DATE}}`, `{{CLOUD_PROVIDER}}`) into the personalized template tree using `find ... -exec sed -i '' -e ... +` (macOS) or `find ... -exec sed -i -e ... +` (Linux). This approach has three structural defects identified during BLP-02 Wave 1 scoping:
