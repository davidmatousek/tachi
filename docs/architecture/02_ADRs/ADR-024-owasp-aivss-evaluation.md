...
**Difference**: AIVSS scores 9.4 (Critical); tachi scores 6.7 (Medium). Same band-crossing pattern as Example 1: AIVSS's amplification of agentic factors pushes a moderate-CVSS finding into Critical, while tachi's weighted sum holds it in Medium. The score gap (9.4 − 6.7 = 2.7) is wider than Example 1's (9.9 − 6.9 = 3.0), demonstrating that the two frameworks cannot be reconciled by a simple offset or scaling factor — the divergence is structural.
