...
| `tachi-pytest.yml` (paths/Rust-suite completeness) | Existing CI workflow (Phase 6 expansion) | `.github/workflows/tachi-pytest.yml` | Modify | F-250 Phase 6 Option Z D-5 + D-6 — (a) add the Rust init test modules to `paths:` filter; (b) add the same Rust modules to the `cargo test` invocation; (c) keep the macOS/Ubuntu matrix to validate bash compatibility across the Rust-owned successors |
